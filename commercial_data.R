...
ACL_wide <- ACLs |>
  ...
  arrange(PRINT_ORDER) |>
  select(total, commercial_total, SECT, CP, rec, Midwater, Scallop, Small_Mesh, State, Other)

EOY_catch_acl <- EOY_catch_wb |>
  wb_add_data(sheet = "ACLs", x = ACL_wide, start_col = 2, start_row = 6, col_names = FALSE, na.string = "")

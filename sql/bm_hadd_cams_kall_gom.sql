INSERT /*+ APPEND*/ INTO  hadd_cams_kall_gom
...
FROM
  (select * from hadd_obdbs_gom where fishing_year = &fy-1)
),hadd_ratio AS(
SELECT
...
FROM( --current year discard rate
  SELECT
    COUNT(distinct vtrserno) obs_trips -- AWA 11.02.21 switched from link1 to vtrserno
  ...
  FROM
    hadd_obdbs_gom
WHERE
  fishing_year = &fy  --ADDED THIS LINE OF CODE ON 7/20/2015 to separate 2015 inseason trips
)
,assumed_rate
), land_plus_disc as(
--BEGIN PRIMARY QUERY
...
AND area IN (464, 465, 511, 512, 513, 514, 515)  --GOM
...
GROUP BY
  ...
  ,hd.bycatch_hadd_ratio
  ,area
  --,d.fishing_year||'_'||d.id
  ,vtrserno -- AWA 11.02.21
  order by permit,date_land)
select *
from land_plus_disc
UNION all --- add VTR discards
SELECT
